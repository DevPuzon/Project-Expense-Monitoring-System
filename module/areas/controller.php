...
function doInsert()
{ 
    // echo implode(",",$_POST['CATEGORY1'])."<br>";
    // echo implode(",",$_POST['AMOUNT1']);
    // return;
    if (isset($_POST['save'])) {
        ...
        $TOTAL_CASH_ADVANCE = $_POST['TOTAL_CASH_ADVANCE'];
        $CASH_ADVANCE_DETAILS = $_POST['CASH_ADVANCE_DETAILS'];

        $areas->PROJECTID = $PROJECTID;
        $areas->LOT_NO = $LOT_NO;
        $areas->BLOCK_NO = $BLOCK_NO;
        $areas->TOTAL_EXPENSES = $TOTAL_EXPENSES;
		$areas->TOTAL_CASH_ADVANCE = $TOTAL_CASH_ADVANCE;
		$areas->CASH_ADVANCE_DETAILS=$CASH_ADVANCE_DETAILS;
        $istrue = $areas->create();
        ...
        foreach ($cur as $r) {
            ...
            $Project->PROJECTCASHADVANCE = $r->PROJECTCASHADVANCE + $TOTAL_CASH_ADVANCE;
...
'$AREAID','$CATEGORY','$SCOPE_OF_WORK','$PCS','$AMOUNT','$TOTAL_AMOUNT')";
        ...
        $item_c = count($_POST['CATEGORY1']);
        for ($i = 0; $i < $item_c; $i++) { 
            $col_amount = $_POST['AMOUNT1'][$i]; 
            $col_cashadv_id = $_POST['CATEGORY1'][$i];  
            $SCOPE_OF_WORK = $_POST['SCOPE_OF_WORK1'][$i];

            global $mydb;
            $statementexp = "INSERT INTO tbl_areacashadv (`SCOPE_OF_WORK`, `col_cashadv_id`, `col_amount`) VALUES (
'$SCOPE_OF_WORK','$col_cashadv_id','$col_amount')";
            $mydb->setQuery($statementexp);
            //$cur = $mydb->loadResultList();
