...
function doAddDExpenses()
{
    ...
    $PCS = $_POST['PCS'];
    $AMOUNT = $_POST['AMOUNT'];
    $TOTAL_AMOUNT = $PCS * $AMOUNT;
     
	  $statementexp = "SELECT SUM(TOTAL_AMOUNT)projectexpenses FROM `tblarea_expenses` WHERE AREAID =" . $AREAID . "";
    $mydb->setQuery($statementexp);
    $cur = $mydb->loadResultList(); 
    foreach ($cur as $r) {
        $statementexp = "UPDATE tblareas SET TOTAL_EXPENSES=' $r->projectexpenses' WHERE AREAID='$AREAID'";
        $mydb->setQuery($statementexp);
    }
	//echo $AREAID;
  //  echo $r->projectexpenses;
 ...
 '$AREAID','$CATEGORY','$SCOPE_OF_WORK','$PCS','$AMOUNT','$TOTAL_AMOUNT')";
    ...
    message("Expenses has been added!", "success");
   redirect("index.php?view=view&id=" . $PROJECTID);
}

function doEditExpenses()
{
    global $mydb;
    $AREAEXPENSEID = $_POST['AREAEXPENSEID'];
    $PROJECTID = $_POST['PROJECTID'];
    $AREAID = $_POST['AREAID'];
    $CATEGORY = $_POST['CATEGORY'];
    $SCOPE_OF_WORK = $_POST['SCOPE_OF_WORK'];
    ...
    $AMOUNT = $_POST['AMOUNT'];
    $TOTAL_AMOUNT = $PCS * $AMOUNT;
    $statementexp = "UPDATE tblarea_expenses SET  CATEGORY='$CATEGORY',SCOPE_OF_WORK='$SCOPE_OF_WORK',PCS='$PCS',AMOUNT='$AMOUNT',TOTAL_AMOUNT='$TOTAL_AMOUNT' WHERE 
AREAEXPENSEID='$AREAEXPENSEID'";
    $mydb->setQuery($statementexp);
